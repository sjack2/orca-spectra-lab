...
#
# Usage:
#   5-orca-boltzmann-weight.sh TAG
#   5-orca-boltzmann-weight.sh --temp 310 --p-cut 0.02 aspirin
#   5-orca-boltzmann-weight.sh --list mols.txt --dry-run
#
# Flags:
...
#   5-orca-boltzmann-weight.sh --temp 310 --p-cut 0.05 --list molecules.txt
#
# ============================================================================
...
extract_energies() {
    ...
    # find all ORCA output files and extract the final energy from each
    while IFS= read -r logfile; do
        local energy_line cid energy
        energy_line=$(grep 'FINAL SINGLE POINT ENERGY' "$logfile" | tail -n1) || true
        [[ -n $energy_line ]] || continue
        energy=$(awk '{print $NF}' <<< "$energy_line")
        cid=$(basename "${logfile%.*}")
        printf '%s\t%s\n' "$cid" "$energy"
    done < <(find "$opt_dir" -type f \( -name '*.log' -o -name '*.out' \) | sort)
}
